...
- (instancetype)initWithMusicModel:(LSMusicModel*)model
{
    ...
    if (self = [super init]) {
        self.first = YES;
        [MBProgressHUD showSuccess:[NSString stringWithFormat:@"%@ 添加到下载队列", model.name]];
        ...
        self.task = [[AFHTTPSessionManager manager] downloadTaskWithRequest:request progress:&progress destination:^NSURL*(NSURL* targetPath, NSURLResponse* response) {
            ...
            completionHandler:^(NSURLResponse* response, NSURL* filePath, NSError* error) {
                ...
                if (error) {
                    ...
                    [MBProgressHUD showSuccess:[NSString stringWithFormat:@"%@ 下载完成", model.name]];
                ...
                self.task = [[AFHTTPSessionManager manager] downloadTaskWithRequest:request progress:&progress destination:^NSURL*(NSURL* targetPath, NSURLResponse* response) {
                    ...
                    completionHandler:^(NSURLResponse* response, NSURL* filePath, NSError* error) {
                        ...
                        if (error) {
                            ...
                            [MBProgressHUD showSuccess:[NSString stringWithFormat:@"%@ 下载完成", self.musicModel.name]];
                            [[LSMusicList musicList] addMusicWithMusicModel:self.musicModel];
                        }
                    }];
                self.progress = progress;
